...
struct table_util
{
    ...
    table_util(size_t num_cols, size_t min_width)
      : cols(num_cols)
    {
    ...
    void print_headers(I headers, size_t col_count, size_t min_width)
    {
        ...
        {
            col_width.push_back(strlen(*headers) +
                (i ? padding : 0));    // no padding before the first column
    ...
    {
        print_headers(
            hdrs_first, std::distance(hdrs_first, hdrs_last), min_width);
    }
    ...
    template <typename I1,
        typename I2>    // value type of I1 is double; value type of I2 = size_t
    void print_row(I1 vals, I2 decimals)
    {
        ...
        {
            std::cout << std::setw(col_width[i]) << std::fixed
                      << std::setprecision(*decimals) << *vals;
        }
        std::cout << std::endl;
    }
    ...
    void print_row(
        I1 vals, I2 decimals, size_t winner_start, bool max_winner = false)
    {
        assert(cols > 0);

        I1 winner = max_winner ?
            std::max_element(vals + winner_start, vals + cols) :
